    ...
    <v-card height="70px" tile>
      ...
      >
        ...
        >
          ...
          <v-avatar>
            <img :src="getImage(region.guid)" :alt="region.name + ' Image'">
          </v-avatar>
        </v-btn>
      </v-bottom-navigation>
    </v-card>
...
<script>
import { regions, populationLevels, getEndProductsAsTree } from '@/data/anno1800params'
...
export default {
  ...
  data () {
    return {
      ...
      hierarchy: getEndProductsAsTree().regions,
      regions: regions,
  ...
  methods: {
    ...
    changeRegion: function (regionGUID) {
      ...
      const regionData = this.hierarchy.find(region => region.guid === this.selectedRegionGUID)

      const selectedPopulation = regionData.populationLevels[0]
